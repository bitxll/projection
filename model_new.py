...
class SRCNN(object):
  ...
  def train(self, config):
    ...
    if config.is_train:
      ...
      h,w=image.shape
      results=image[:self.n_steps,:]
      for i in range(0,(h-self.n_steps),self.out_step):
        sub_input=image[i:self.n_steps+i,:]
        sub_input=np.array([sub_input])
	#eval():将字符串str当成有效的表达式来求值并返回计算结果
        result = self.pred.eval({self.images: sub_input})
	#result = self.sess.run(self.pred,{self.images:train_data})
        results=np.vstack((np.squeeze(results),result))
      ...
      scipy.io.savemat(image_path_label,{'R':image})
